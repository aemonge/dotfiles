...
-- import XMonad.Layout.Magnifier  -- layoutMagnifier windows will overlap
...
myFocusedBorderColor = "#8F5536"      -- color of focused border
myNormalBorderColor  = "#151515"      -- color of inactive border
...
myTitleColor     = "#EFE7BE"  -- color of window title
myCurrentWSColor = "#0094CE"  -- color of active workspace
myVisibleWSColor = "#cecece"  -- color of inactive workspace
myUrgentWSColor  = "#68200A"  -- color of workspace with 'urgent' window
...
-- space for the status bar at the top of the screen.
	-- layoutMagnifier windows will overlap
-- defaultLayouts = smartBorders(avoidStruts( magnifier(
...
{-chatLayout = avoidStruts(withIM (1%7) (Title myIMRosterTitle) Circle)-}
...
myKeyBindings =
  [
	...
	, ((myModMask , xK_i), spawn "transset-df -a --dec .02")
    , ((myModMask , xK_o), spawn "transset-df -a --inc .02")
    , ((myModMask , xK_u), spawn "transset-df -a -t")
    , ((0, 0x1008FF12), spawn "amixer -q set Master toggle")
    , ((0, 0x1008FF11), spawn "amixer -q set Master 6%-")
    , ((0, 0x1008FF13), spawn "amixer -q set Master 6%+")
    -- , ((myModMask , xK_f), sendMessage MagnifyMore)
    -- , ((myModMask , xK_s), sendMessage MagnifyLess)
    -- , ((myModMask , xK_d), sendMessage Toggle     )
    , ((myModMask , xK_r), spawn "xcompmgr -n")
    , ((myModMask , xK_g), spawn "nautilus --no-desktop")
  ]
...
myManagementHooks = [
  ...
  , (className =? "Gimp-2.8") --> doF (W.shift "5:Pix")
  ,  (ask >>= \w -> liftX (setOpacity w 0.92) >> idHook)
  ,  (ask >>= \w -> liftX (setOpacity w 0.92) >> idHook)
  ]
...
main = do
  {-xmproc <- spawnPipe "xmobar --screen 1 ~/.xmonad/xmobarrc"-}
